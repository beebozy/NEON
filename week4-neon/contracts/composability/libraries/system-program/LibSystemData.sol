...
// with this really return the way it should return a random32 bytes account 
    function getCreateWithoutSeedAccount(
    bytes32 programId,
    uint256 nonce
) internal pure returns (bytes32) {
    return sha256(abi.encodePacked(programId, nonce));
}
    /// @param accountPubKey The 32 bytes Solana account public key
