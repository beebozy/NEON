...
/// @notice Helper function to format a `createAccount` instruction (without seed)
/// @param payer The payer account which will fund the newly created account
/// @param newAccount The new account public key to be created
/// @param programId The `id` of the Solana program which will be granted permission to write data to the newly created account
/// @param accountSize The on-chain storage space for the newly created account
function formatCreateAccountWithoutSeedInstruction(
    bytes32 payer,
    bytes32 newAccount,
    bytes32 programId,
    uint64 accountSize
    ...
    // Set up accounts array (payer, new account)
    accounts = new bytes32[](2);
    accounts[0] = payer;
    accounts[1] = newAccount;

    // Set up signers array (payer must sign)
    isSigner = new bool[](2);
    isSigner[0] = true;
    isSigner[1] = true; // new account keypair must sign for creation

    // Set up writable flags
    isWritable = new bool[](2);
    isWritable[0] = true; // payer is writable (lamports will be deducted)
    isWritable[1] = true; // new account is writable
    ...
    bytes8 lamportsLE = bytes8(SolanaDataConverterLib.readLittleEndianUnsigned64(rentExemptionBalance));
    bytes8 spaceLE = bytes8(SolanaDataConverterLib.readLittleEndianUnsigned64(accountSize));

    // Pack the instruction data
    data = abi.encodePacked(
        bytes4(0x00000000), // System Program instruction discriminator for CreateAccount
        lamportsLE,         // Rent exemption balance in lamports (little-endian)
        spaceLE,            // Account size in bytes (little-endian)
        programId           // Program ID that will own the new account
    );
}
